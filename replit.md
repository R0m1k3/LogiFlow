...
- **Information System (Announcements):** Système d'informations complet remplaçant la carte commandes clients dans le dashboard - gestion d'annonces avec niveaux de priorité (normal, important, urgent), contrôle d'accès basé sur les rôles (admin : CRUD complet, autres rôles : lecture seule), tri intelligent par priorité et date, filtrage par magasin pour administrateurs, interface moderne avec badges colorés et indicateurs visuels. **STOCKAGE MÉMOIRE (Août 2025)** : Migration du stockage SQL vers stockage en mémoire partagé avec limite de 5 annonces maximum pour éviter tout problème de base de données et simplifier l'architecture.
