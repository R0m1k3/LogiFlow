...
export class DatabaseStorage implements IStorage {
      ...
      const [cache] = await db
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...
            invoiceReference: cacheData.invoiceReference,
            invoiceAmount: cacheData.invoiceAmount,
            dueDate: cacheData.dueDate,
            isReconciled: cacheData.isReconciled,
  ...
  async getDlcProducts(groupIds?: number[], filters?: { status?: string; supplierId?: number; search?: string; }): Promise<DlcProductWithRelations[]> {
    ...
    // Tri intelligent : produits actifs (non grisés) EN PREMIER, puis lignes grisées en dernier
    const results = await query.orderBy(
      // Priorité : Actifs → Traités → Stock épuisé → Validés
      sql`CASE 
        WHEN ${dlcProducts.status} != 'valides' 
             AND (${dlcProducts.processedUntilExpiry} IS NULL OR ${dlcProducts.processedUntilExpiry} = false)
             AND (${dlcProducts.stockEpuise} IS NULL OR ${dlcProducts.stockEpuise} = false) THEN 1
        WHEN ${dlcProducts.processedUntilExpiry} = true THEN 2
        WHEN ${dlcProducts.stockEpuise} = true THEN 3
        WHEN ${dlcProducts.status} = 'valides' THEN 4
        ELSE 1
      END`,
...
export class MemStorage implements IStorage {
  ...
  async createInvoiceVerificationCache(cache: InsertInvoiceVerificationCache): Promise<InvoiceVerificationCache> {
    const newCache: InvoiceVerificationCache = {
      ...
      invoiceAmount: cache.invoiceAmount || null,
      dueDate: cache.dueDate || null,
      exists: cache.exists,
      matchType: cache.matchType,
      errorMessage: cache.errorMessage || null,
      isReconciled: cache.isReconciled || false,
